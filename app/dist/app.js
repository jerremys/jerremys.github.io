...
latin = {
    ...
    buildCommonIndex: function (commonList) {
        if (commonList === void 0) { commonList = []; }
        if (this.roots.length > 0) {
            console.log("Build common indexes");
            var stripper = function (val) {
                return val[0];
            };
    ...
    handleSettingChanged: function (el) {
        var $el = jquery_1["default"](el.currentTarget), name = $el.prop("name");
    ...
    nextRoot: function (transition) {
        if (transition === void 0) { transition = true; }
        var newRootIndex = this.getNextIndex(), newRoot = this.roots[newRootIndex];
        var delay = 0;
        this.toggleHint();
    ...
    renderAnswers: function (rootIndex) {
        var answerHtml = '', picked = [rootIndex], container = jquery_1["default"]('#answerContainer'), root = this.roots[rootIndex];
...
var db = {
    ...
    showWordStats: function (wordStats) {
        var guessessAsc = new Map(__spreadArrays(wordStats.entries()).sort(function (a, b) { return b[1].guesses - a[1].guesses; }));
        var guessessDesc = new Map(__spreadArrays(wordStats.entries()).sort(function (a, b) { return a[1].guesses - b[1].guesses; }));
        var wordsAsc = new Map(__spreadArrays(wordStats.entries()).sort(function (a, b) { return a[1].occurances - b[1].occurances; }));
        var wordsDesc = new Map(__spreadArrays(wordStats.entries()).sort(function (a, b) { return b[1].occurances - a[1].occurances; }));
    ...
    renderWordStats: function ($container, valueKey, wordMap) {
        ...
        for (var _i = 0, wordMap_1 = wordMap; _i < wordMap_1.length; _i++) {
            var _a = wordMap_1[_i], key = _a[0], value = _a[1];
    ...
    showChart: function (dayStats) {
        ...
        var chartQuestions = [], chartAnswers = [], timeTotals = [], secondsPerQuestion = [], averageGuesses = [];
...
jquery_1["default"](window.document).ready(this.init);
